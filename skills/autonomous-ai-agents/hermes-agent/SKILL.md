...
| `memory` | `memory_enabled`, `user_profile_enabled`, `provider`, `memory_char_limit` (2200), `user_char_limit` (1375) — see `references/memory-limits.md` |
...
### Adding a Custom Web Search Provider

Want Hermes to use a search engine not in the built-in list (e.g., 博查, 本地搜索API)?
See `references/creating-web-providers.md` for the complete step-by-step — from plugin
files to source patches to testing. Covers the `WebSearchProvider` ABC, response shape
contract, and integration checklist.

### Backing Up Hermes Data (GitHub + Gitee)

If Hermes runs on a cloud server, back up sessions, memory, config, and skills to
GitHub + Gitee with a daily cron job. See `references/hermes-backup-setup.md` for the
full workflow: SSH key setup, dual-remote git repos, backup script, and cron scheduling.
Critical: never show terminal output or technical details to non-technical users — handle
everything server-side and only ask them to create repos and paste SSH keys.
